let currentBlocks = [];
        ...
        function parseMarkdown() {
            ...
            const makeParagraph = () => {
                if (currentParagraph.length > 0) {
                    ...
                    newBlocks.push({ type: 'paragraph', content: content });
                    currentParagraph = [];
                }
            };
            ...
            lines.forEach(line => {
                ...
                if (trimmed.startsWith('# ')) {
                    makeParagraph();
                    newBlocks.push({ type: 'heading', level: 'h1', content: trimmed.substring(2) });
                } else if (trimmed.startsWith('## ')) {
                    makeParagraph();
                    newBlocks.push({ type: 'heading', level: 'h2', content: trimmed.substring(3) });
                } else if (trimmed.startsWith('### ')) {
                    makeParagraph();
                    newBlocks.push({ type: 'heading', level: 'h3', content: trimmed.substring(4) });
                } else if (trimmed.startsWith('![')) {
                    makeParagraph();
                    const imageMatch = trimmed.match(/!\[([^\]]*)\]\(([^)]+)\)/);
                    if (imageMatch) {
                        newBlocks.push({ type: 'image', src: imageMatch[2], alt: imageMatch[1] });
                    }
                } else if (trimmed === '') {
                    makeParagraph();
                } else {
        ...
        function selectBlock(blockId, event) {
            ...
            const isMultiSelect = event.ctrlKey || event.metaKey; // Ctrl or Cmd key
        ...
        function updateProperty(property, value) {
            if (selectedBlock) {
                ...
                renderBlocks();
                // Re-select the block to maintain selection
                setTimeout(() => selectBlock(selectedBlock.id), 50);
            }
        }
